...
Notice the branch and chevron symbols in the screenshot above? Those are not in the standard character set, so we need a patched font so that they display properly. I use the MesloLGS font from https://github.com/powerline/fonts/tree/master/Meslo%20Slashed[here, window='_blank']. Just download and install the .ttf files.
...
I use iTerm2's built-in glyphs in iTerm2 as I feel they render better, and patched/nerd font in Visual Studio Code terminals.
